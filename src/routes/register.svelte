<script lang="ts">
	...
	const handleSubmit = async () => {
		try {
			isLoading = true;
			await register({ name, email, password, confirmPassword, username });
			goto('/app/');
		} catch (error) {
			console.log(error);
		}
...
>
	<div class="flex flex-col gap-2">
		...
		<Input placeholder="Email" label="Email" type="email" name="email" bind:value={email} />
		<Input
			...
			bind:value={username}
		/>
		<Input placeholder="Name" label="Name" type="text" name="name" bind:value={name} />
		<Input
	...
	<div class="max-w-xl flex flex-col gap-2">
		<Button {isLoading} type="submit">Register</Button>
	</div>
</form>
